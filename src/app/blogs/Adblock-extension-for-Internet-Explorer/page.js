...
const Blog_2 = () => {
    const data = {
        ...
        text1: "Regretfully, Internet Explorer's plugin working method is pretty particular and not designed for creating <a href='/' class='text-blue-300'>Trueadblocker extensions</a>, unlike the browsers mentioned above. Furthermore, creating plugins for Internet Explorer is challenging. Due to these factors, Internet Explorer was circumvented by various plugins, extensions, and addons intended for ad blocking. While there are many excellent adblockers (like TrueAdblock extension, TrueAdblocker Plus, No Ads, and others) that block pop-ups and remove ads on other browser platforms, there is currently only one TrueAdblocker that, despite its limitations, functions: the TrueAdblocker Plus extension for Internet Explorer. <br/> <br/> Unfortunately, there are several significant drawbacks to this ad blocker. The following issues with TrueAdblocker + for Internet Explorer have been reported by the creators themselves on the official website:",
        list: {
